function initiate() {
  ...
  if (
    !document.querySelector("#Jon").checked &&
    !document.querySelector("#Maple").checked
  )
    document.querySelector("#Jon").click(); //if none checked, make Jon default
  gameMode();
  ...
  if (!navigator.userAgent.match(/Trident\/7\./)) {
    ...
    document.getElementById("TempScore").textContent =
      "This game doesn't work on Internet Explorer. Get off it you dinosaur!";
  }
  document.onkeydown = function (e) {
    var keyCode = e.which || e.keyCode;
    var handled = false;
    if (keyCode == 38 || keyCode == 40 || keyCode == 32) {
      //up or down or spacebar
      //do something fun
      e.preventDefault();
      handled = true;
    }
...
function gameMode() {
  ...
  if (document.querySelector("#Jon").checked) {
    document.querySelector("#Desc").innerText =
      "Help Jon make it safely through the day. Press spacebar to start.";
    ...
    document.querySelector("#Desc").innerText =
      "Help the noob survive. Press spacebar to start.";
