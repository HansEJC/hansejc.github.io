...
function saveScores(scr) {
  if (!window.location.hostname.includes("github")) {
    ...
    scoreID = document.cookie.includes("game=dobble")
      ? "dobbleScores"
      : scoreID;
    let scoreArr = [];
...
async function getScores(file, storage) {
  const serverScore = await fetch(file).then((result) => result.json());
...
function speech() {
  try {
    ...
    console.log(err + " speech recognition not supported");
  }
}
