async function delivery() {
  ...
  DRw = [];
  DR = [];
  let DRpcsv = await fetch("Orion Park/op-Project Stock.csv").then((result) =>
    result.text()
  );
  let DRwcsv = await fetch(
    "Orion Park/op-Warehouse Stock .csv"
  ).then((result) => result.text());
  let DRcsv = await fetch("Orion Park/op-Delivery.csv").then((result) =>
    result.text()
  );
  let lastmod = await fetch("Orion Park/op.xlsx").then((result) =>
    result.headers.get("Last-Modified")
  );
  let lastfetch = await fetch("Orion Park/op-Delivery.csv").then((result) =>
    result.headers.get("Last-Modified")
  );
  // code below here will only execute when await fetch() finished loading
  document.getElementById("p").textContent =
    "Last Updated: " + new Date(lastmod);
  document.getElementById("pp").textContent =
    "Last Checked: " + new Date(lastfetch);
  ...
  for (let i = 0; i < DRp.length; i++) {
    if (
      DRp[i][5] == "" ||
      new RegExp("date", "i").test(DRp[i][5]) ||
      new RegExp("undefined", "i").test(DRp[i][5])
    ) {
      DRp.splice(i, 1);
      i--;
    }
  ...
  for (let i = 0; i < DRw.length; i++) {
    if (
      DRw[i][7] == "" ||
      new RegExp("location", "i").test(DRw[i][7]) ||
      new RegExp("undefined", "i").test(DRw[i][7])
    ) {
      DRw.splice(i, 1);
      i--;
    }
  ...
  for (let i = 0; i < DR.length; i++) {
    if (
      DR[i][0] == "" ||
      new RegExp("date", "i").test(DR[i][0]) ||
      new RegExp("workshop", "i").test(DR[i][0])
    ) {
      DR.splice(i, 1);
      i--;
    }
...
function search(arrheh) {
  ...
  sArray = myArray
    .filter((s) => pn.every((v) => s.toLowerCase().includes(v)))
    .map((e) => {
      if (pn[0].length > 0)
        e = e.replace(
          new RegExp(RegExp.quote(pn.join("|")), "gi"),
          (x) => "<mark>" + x + "</mark>"
        );
      return e.split(",");
    });
  ...
  if (document.getElementById("Del").checked) {
    myTable =
      "<table><col width='10'><col width='10'><col width='10'><col width='10'><col width='10'><col width='10'><tr><th>Date</th>";
    myTable += "<th>Time</th>";
    ...
    //create table
    myTable =
      "<table><col width='10'><col width='10'><col width='10'><col width='10'><col width='10'><tr><th>Part #</th>";
