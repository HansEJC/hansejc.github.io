...
function cardsGone() {
  gamesStarted = false;
  document.getElementById("TempScore").innerHTML =
    "No more cards left! Press Start to continue.";
...
function clicked(e) {
  ...
  HouseDeck.forEach((card) => {
    if (
      classy[classy.length - 1] == card.classList[card.classList.length - 1]
    ) {
      k(10, 500, 50);
...
function numPlayers() {
  ...
  let x = document.getElementById("Player2"),
    ...
    z = document.getElementById("GameTimer");
  if (this.id == "SingleP")
