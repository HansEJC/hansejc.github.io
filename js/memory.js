...
function clicked(e) {
  ...
  if (
    JSON.stringify(clickArr) == JSON.stringify(colArr) &&
    clickArr.length == colArr.length
  ) {
    setTimeout(() => {
...
function gameOver() {
  ...
  setTimeout(() => {
    k(10, 40, 300);
    board.classList.remove("blue", "red");
  }, 300);
  setTimeout(() => {
    k(10, 30, 300);
    board.classList.add("blue", "red");
    board.classList.remove("green", "orange");
  }, 600);
  setTimeout(() => {
    k(10, 20, 300);
...
function randomColor() {
  ...
  if (num == oldernum) (oldernum = null), randomColor();
  //only two colors in a row
  else innerRandom();
