...
function addOption(opt, desc, bool, plotter) {
  ...
  label.appendChild(newopt);
  function updateOps(e) {
    eval("g3.updateOptions({" + e.target.id + ":" + e.target.checked + "});");
  }
  function updatePlotter(e) {
    if (e.target.checked)
      eval("g3.updateOptions({" + e.target.id + ":" + plotter + "});");
    else
      eval(
        "g3.updateOptions({" + e.target.id + ": Dygraph.Plotters.linePlotter});"
      );
  }
...
function inputsNfunc(db) {
  ...
  document.getElementById("99").onchange = function () {
    saveRadio(this);
    read(db);
  };
  document.getElementById("dat").onblur = function () {
    saveValue(this);
    read(db);
  };
  document.getElementById("datR").onblur = function () {
    saveValue(this);
    read(db);
  };
  document.getElementById("eqcheck").onchange = function () {
    saveRadio(this);
    read(db);
  };
  ...
  document.forms["myForm"].elements["my_upload"].onchange = function (evt) {
    ...
    reader.onload = function (evt) {
      ...
      if (document.getElementById("CSVF").checked) {
        try {
          //the for loop removes saved labels
          ...
          console.log(err);
          addLoader(
            "CSV Formatting Error, Attempting to Process Upload.",
            true
          );
        }
      }
  ...
  try {
    ...
    objectStore.openCursor(null, "prev").onsuccess = async function (event) {
      try {
        ...
        var csv = await fetch("uploads/graph.csv").then((result) =>
          result.text()
        );
        plotcalcs(csv, db);
      }
...
function plotcalcs(file, db) {
  ...
  for (let i = 0; i < csv.length; i++) {
    //make all rows have equal numbers to bottom row
...
function plotexp(file) {
  ...
  if (document.getElementById("dat").value != "")
    dat = new Date(document.getElementById("dat").value);
  if (document.getElementById("datR").value != "")
    datrate = Number(document.getElementById("datR").value) * 1000;
  if (document.getElementById("99").checked) {
    //set own start date and increment
  ...
  try {
    while (document.getElementById("equa").childElementCount > 2) {
      //don't remove the firstborn children
      document
        .getElementById("equa")
        .removeChild(document.getElementById("equa").lastChild);
    }
  } catch (err) {}
  ...
  for (let i = 1; i < csv[0].length; i++) {
    ...
    equ[i].type = "text";
    equ[i].value =
      getSavedValue(String.fromCharCode(96 + i)) == ""
        ? String.fromCharCode(96 + i)
        : getSavedValue(String.fromCharCode(96 + i));
    //eqw[i-1] = equ[i].value;
    equ[i].id = String.fromCharCode(96 + i);
    equ[i].onkeyup = function () {
      saveValue(this);
    };
  ...
  function arrayequations() {
    for (let i = 0; i < csv.length; i++) {
      for (let j = 1; j < csv[i].length; j++) {
        ...
        csv[i][j] = eval(
          document.getElementById(String.fromCharCode(96 + j)).value
        );
      }
    }
  }
  document.getElementById("eqcheck").checked =
    getSavedValue("eqcheck") == "true";
  if (document.getElementById("eqcheck").checked) {
    eqb = true;
...
function dyg(csv) {
  ...
  try {
    ...
    while (myNode.childElementCount > 3) {
      //don't remove the firstborn children
      myNode.removeChild(myNode.lastChild);
      while (document.getElementById("ColorForm").childElementCount > 1) {
        //don't remove the firstborn children
        document
          .getElementById("ColorForm")
          .removeChild(document.getElementById("ColorForm").lastChild);
      }
    }
  } catch (err) {}
  try {
        ...
        x: {
          axisLabelFormatter: function (y, gran, opts) {
            return y instanceof Date
              ? Dygraph.dateAxisLabelFormatter(y, gran, opts)
              : smoothdec(y);
          },
        },
        y: {
...
function dygReady() {
  ...
  var cb = [],
    cb2 = [],
    col = [];
  const cbh = document.getElementById("MyForm"),
    colF = document.getElementById("ColorForm");

  for (var i = 0; i < lbs.length; i++) {
    cb[i] = document.createElement("input");
    cb2[i] = document.createElement("input");
    col[i] = document.createElement("input");
    cb[i].type = "checkbox";
    cb2[i].type = "text";
    col[i].type = "color";
    cbh.appendChild(cb[i]);
    cbh.appendChild(cb2[i]);
    colF.appendChild(col[i]);
    cb[i].id = "csvcheckbox " + i;
    cb2[i].id = "csvlabel" + i;
    col[i].id = "csvcolor" + i;
    cb2[i].value = lbs[i];
  ...
  setTimeout(function () {
    ...
    document.getElementById("myForm").innerHTML =
      "<input type='file' accept='.csv' id='my_upload' name='my_upload'/><span id='FullFile'></span></b>"; // Resets input to be able to upload same file.
    document.querySelector("#FullFile").innerText = localStorage.getItem(
      "Filename"
    );
    startup(true);
  }, 500);
