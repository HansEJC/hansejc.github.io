function calculations() {
  ...
  var cb2 = [];
  var cb3 = [];
  var cbh = document.getElementById("Parallel");
  var cbh2 = document.getElementById("Nums");
  var cbh3 = document.getElementById("Angs");
  if (npr < 100) {
    for (var i = 0; i < npr; i++) {
      cb[i] = document.createElement("input");
      cb2[i] = document.createElement("input");
      cb3[i] = document.createElement("input");
      cb[i].type = "number";
      cb2[i].type = "number";
      cb3[i].type = "number";
      cbh.appendChild(cb[i]);
      cbh2.appendChild(cb2[i]);
      cbh3.appendChild(cb3[i]);
      cb[i].id = i;
      cb2[i].id = i + npr;
      cb3[i].id = i + 2 * npr;
    }
    ...
    cb2[0] = document.createElement("input");
    cb3[0] = document.createElement("input");
    cb[0].type = "text";
    cb2[0].type = "text";
    cb3[0].type = "text";
    cb[0].value = "bad human!";
    cb2[0].value = "bad human!";
    cb3[0].value = "bad human!";
    cbh.appendChild(cb[0]);
    cbh2.appendChild(cb2[0]);
    cbh3.appendChild(cb3[0]);
  }
}
...
function polrec() {
  ...
  document.getElementById("PTR").textContent =
    +ptrr.toFixed(2) + " + j" + Number(ptir.toFixed(2));
  document.getElementById("RTP").textContent =
    +rtpn.toFixed(2) + " + ∠" + Number(rtpa.toFixed(2)) + "°";
}
...
function poladd() {
  ...
  if (reals < 0 && imags < 0)
    ang = -180 + (180 * Math.atan(imags / reals)) / Math.PI;
  document.getElementById("PSUM").textContent =
    +num.toFixed(2) + " + ∠" + Number(ang.toFixed(2)) + "°";
}
...
function loadcalc() {
  ...
  var nl = Math.sqrt(
    l1l * l1l + l2l * l2l + l3l * l3l - l1l * l2l - l2l * l3l - l1l * l3l
  );
  ...
  if (tpow > 1000000)
    document.getElementById("TPOW").textContent =
      +(tpow / 1000000).toFixed(2) + " MVA";
  else if (tpow > 1000)
    document.getElementById("TPOW").textContent =
      +(tpow / 1000).toFixed(2) + " kVA";
...
function consum() {
  ...
  document.getElementById("COND").textContent =
    +cond.toFixed(0) + " days " + conm.toFixed(0) + " hours";
