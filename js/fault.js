function quantities() {
  ...
  if (NumLocs < 20) {
    for (let i = 0; i < NumLocs; i++) {
      ...
      cb2[i].type = "number";
      cb3[i].type = "number";
      cb[i].onkeyup = function () {
        saveValue(this);
      };
      cb2[i].onkeyup = function () {
        saveValue(this);
      };
      cb3[i].onkeyup = function () {
        saveValue(this);
      };
      myNode.appendChild(cb[i]);
      myNode2.appendChild(cb2[i]);
      if (i < NumLocs - 1) myNode3.appendChild(cb3[i]);
      cb[i].id = i + 200;
      cb2[i].id = i + 100;
      cb3[i].id = i + 300;
      cb[i].classList.add("sub");
      cb2[i].classList.add("loc");
      cb3[i].classList.add("sub");
    }
  } else {
    ...
    myNode.appendChild(cb[0]);
    myNode2.appendChild(cb2[0]);
  }
}
...
function calculations() {
  ...
  let totlc = 0,
    ...
    previmpneg = 0,
    prevoleneg = 0,
    textlc = 0; //total length, previous impedance, prev OLE
  ...
  document
    ...
    .forEach((sub) => (sub.value = getSavedValue(sub.id)));
  document.querySelectorAll(".loc").forEach((loc, ind) => {
    ...
    for (let i = 1; i <= res; i++) {
      ...
      let lxb =
        smoothdec(lcc % nxbnd) == 0 ? nxbnd : smoothdec(lcc % nxbnd) || 0; //location after last xbond
      ...
      if (boost)
        ({ oleimp, returnimp } = boosterCalc({ oleimp, returnimp, ...stuff }));
      ...
      if (getSavedValue(loc.id) < 0) {
        //negative sub locs
        subfault =
          vol / (faultimp + imp + previmp + prevole + previmpneg + prevoleneg);
