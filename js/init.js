...
(function () {
  ...
  darkToggle.addEventListener("change", function () {
    toggleDarkTheme(darkToggle.checked);
    saveCheckbox(this);
  });

  toggleDarkTheme(
    prefersDark.matches && getSavedValue(darkToggle.id) != "false"
  );
...
function navBar() {
  ...
  navbar.innerHTML = `
    ...
    `;
  document.querySelectorAll(".heh").forEach(function (item) {
    return item.addEventListener("click", randomPage);
  });
}
...
const randomChild = function (len) {
  return Math.floor(Math.random() * len);
};
function randomPage(e) {
  test = e;
  if (
    navs.some(function (nav) {
      return e.target.innerHTML.includes(nav);
    })
  ) {
    let nodeLen = randomChild(
      e.target.parentElement.children[1].querySelectorAll("a").length
    );
...
if ("serviceWorker" in navigator) {
  ...
  apple.href = "images/apple-icon-180.png";
  theme.content = "#800080";
  head.appendChild(mani);
  ...
  document
    ...
    .setAttribute(
      ...
      "height:" +
        ...
        "px !important; width:" +
        x +
        "px !important; margin: 0px 0px 10px 10px;"
    );
  window.dispatchEvent(new Event("resize"));
  for (var j = 0; j < 3; j++) {
    //weird way to make it actually work
  ...
  if (typeof navigator.clipboard == "undefined") {
    let htmltext =
      ...
      !navigator.userAgent.includes("Edg")
        ? "<br><br><a href=chrome://flags/#unsafely-treat-insecure-origin-as-secure>Auto copy to clipboard not supported in http. Copy this link and open in new tab to add this site as trusted to enable.</a>"
        : "<br><br><a>Auto copy to clipboard not supported. Right click plot and copy as image.</a>";
...
function saveRadio(e) {
  ...
  document
    ...
    .forEach((rad) => localStorage.setItem(rad.id, rad.checked));
  document
