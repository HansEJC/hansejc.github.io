function startup() {
  ...
  var cb2 = [];
  var cb3 = [];
  var cbh = document.getElementById("Dist");
  var cbh2 = document.getElementById("Stan");
  var cbh3 = document.getElementById("Stiv");
  ...
  for (var i = 0; i < measurements; i++) {
    ...
    cb3[i] = document.createElement("span");
    cb[i].type = "number";
    cb2[i].type = "number";
    cb2[i].step = "0.01";
...
function def2() {
  ...
  var cb = [];
  var cb2 = [];
  var cbh = document.getElementById("Dist2");
  var cbh2 = document.getElementById("Meas");
  for (var i = 0; i < 10; i++) {
    cb[i] = document.createElement("input");
    cb2[i] = document.createElement("input");
    cb[i].type = "number";
    ...
    document.getElementById("fopdis" + i).value = +(fopdis * (i / 10)).toFixed(
      1
    );
  }
...
function fetchResults() {
  try {
    (async () => {
      const soilResults = await fetch("uploads/soil.json").then((result) =>
        result.json()
      );
      soilResults.sort((a, b) => b[1] - a[1]);
      const soilTable = document.getElementById("soilResults");
      resultsTable(soilResults, soilTable);
      const fopResults = await fetch("uploads/fop.json").then((result) =>
        result.json()
      );
...
function table(rows, target) {
  ...
  if (rows.results[0].length > 2)
    row.insertCell(2).outerHTML = "<th>Resistivity (Ωm)</th>";
  ...
  try {
    rows.results.forEach((arr) => {
      ...
      if (arr.length > 2)
        row.insertCell(2).innerHTML = parseFloat(arr[2]).toFixed(2);
    });
  } catch (err) {
    console.log(err);
  }
  ...
  if (target.checked) document.querySelector("#tab").appendChild(myTable);
    ...
    document
      .querySelector("#tab")
      .removeChild(document.querySelector("#tab").lastChild);
}
