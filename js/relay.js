...
function read() {
  ...
  objectStore.openCursor(null, "prev").onsuccess = async function (event) {
    ...
    try {
      ...
      let DRcsv = await fetch("uploads/fault.csv").then((result) =>
        result.text()
      );
...
async function startup() {
  ...
  document.getElementById("Prim").onchange = function () {
    read();
  };
  document.getElementById("Sec").onchange = function () {
    checkit();
    read();
  };
  document.getElementById("PrimDR").onchange = function () {
    read();
  };
  document.getElementById("SecDR").onchange = function () {
    read();
  };
}
...
function Zone3(tr) {
  ...
  let Z3el = [
    [-Z3rev * Math.cos(Z3A), , , , , -Z3rev * xmul1],
    [Z3 * Math.cos(Z3A), , , , , Z3 * xmul1],
    [pgr3, , , , pgx3],
    [pkr3, , , , pkx3],
    [pcr3, , , , pcx3],
    [prr3, , , , prx3],
...
function FaultZone(stuff) {
  ...
  for (let i = 0; i < faultarray.length; i++) {
    //check through fault if in zone
