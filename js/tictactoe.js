...
let firstMid = false,
  firstCor = false,
  firstEd = false;
const classInc = (sqr, cls) =>
  squares[sqr].firstChild.classList.value.includes(cls);
...
function checker(three, type, row) {
  if (
    three.every((ev) => ev.includes("X")) ||
    three.every((ev) => ev.includes("O"))
  ) {
...
const toggleMode = () =>
  (mode.textContent = mode.textContent === "Hard" ? "Easy" : "Hard");
...
function nextMove() {
  ...
  square =
    mode.textContent === "Hard"
      ? squares[minimax(board, "O").index].firstChild
      : getMove();
...
const Players = () =>
  document.querySelector("#SingleP").checked ? "Xplay" : togglePlayer();
const togglePlayer = () =>
  (currPlayer = currPlayer === "Xplay" ? "Oplay" : "Xplay");
...
function bonk(e) {
  ...
  try {
    if (
      ["Xplay", "Oplay"].some((ev) =>
        this.firstChild.classList.value.includes(ev)
      )
    )
      return;
...
function twoSimp(sq1, sq2, sq3) {
  if (
    ...
    return squares[sq3 - 1].firstChild;
}
