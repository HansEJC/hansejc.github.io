function firstMortgage(info) {
  ...
  for (let i = 0; i < 13; i++) {
    ...
    teq = teq + eqp;
    mortgage.push([
      new Date(2017, sd.getMonth() + i, sd.getDay()),
      intr,
      eqp,
      intr + eqp,
    ]);
...
function secondMortgage(info) {
  let today = new Date();
  const monthd =
    ((today - new Date(2020, 7, 1)) / 1000 / 60 / 60 / 24 / 365.25) * 12; //difference in month
  ...
  for (let i = 0; i < monthd; i++) {
    // changed overpayment to £1500
    intr = rl * nir;
    eqp = 1500 - intr;
    teq = teq + eqp;
...
function calculations() {
  ...
  document.getElementById("RT").textContent =
    Math.floor(pn / 12) +
    " y " +
    (12 * (pn / 12 - Math.floor(pn / 12))).toFixed(0) +
    " m";
  dygPlot(mortgage);
}
