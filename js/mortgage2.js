function firstMortgage(info){
	const sd = new Date(2017,04,01); //start of mortgage
	const iir = 4.09/12/100; //initial interest rate
	({intr,eqp,teq,mortgage,rl,tpa,tp,pn} = info);
...
function calculations(){	
	...
	info = {intr,eqp,teq,mortgage,rl,tpa,tp,pn} = firstMortgage(info);
	info = {intr,eqp,teq,mortgage,rl,tpa,tp,pn,monthd} = secondMortgage(info);
	({intr,eqp,mortgage,monthd,tp,pn} = futureMortgage(info));
